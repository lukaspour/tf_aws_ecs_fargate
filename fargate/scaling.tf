...
resource "aws_appautoscaling_target" "ecs_target" {
  ...
  resource_id        = "service/${var.cluster_id}/${each.key}"
...
resource "aws_appautoscaling_policy" "dynamodb_table_read_policy" {
  ...
  name               = "${each.key}:${lookup(var.containers_definitions[each.key], "scaling_metric", "ECSServiceAverageCPUUtilization")}:${aws_appautoscaling_target.ecs_target.resource_id}"
